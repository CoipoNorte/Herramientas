...
class DragDropProjectOrganizer(QMainWindow):
    ...
    def add_structure_to_workspace(self, structure_name):
        ...
        if structure_name in self.structures_buttons:
            self.create_folders(self.structures_buttons[structure_name], self.workspace.invisibleRootItem())
    ...
    def delete_folder(self):
        ...
        if selected_item:
            ...
            root_item.removeChild(selected_item)
    
    def show_context_menu(self, pos):
        item = self.workspace.itemAt(pos)
        if item:
            menu = QMenu(self)
            create_action = QAction("Crear Carpeta", self)
            rename_action = QAction("Renombrar Carpeta", self)
            ...
            menu.exec_(self.workspace.viewport().mapToGlobal(pos))
