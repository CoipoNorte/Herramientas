...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QSplitter, QTreeWidget, QTreeWidgetItem,
                             ...
                             QMenu, QAction, QInputDialog, QFileDialog, QAbstractItemView)
from PyQt5.QtCore import Qt, QMimeData
from PyQt5.QtGui import QDrag, QDragMoveEvent, QDropEvent
...
class DragDropProjectOrganizer(QMainWindow):
    ...
    def delete_folder(self):
        ...
        selected_item = self.workspace.currentItem()
        if selected_item:
            reply = QInputDialog.getText(self, "Eliminar Carpeta", "¿Seguro que quieres eliminar esta carpeta? (sí/no)")
            if reply[0].lower() == 'sí':
                self.delete_item_from_disk(selected_item)
                root_item = self.workspace.invisibleRootItem()
                index = root_item.indexOfChild(selected_item)
                if index != -1:
                    root_item.takeChild(index)
                else:
                    self.workspace.removeItemWidget(selected_item, QTreeWidgetItemIterator.All)
    ...
    def show_context_menu(self, pos):
        ...
        new_file_action.triggered.connect(self.create_file_from_context_menu)
    ...
    def create_file_from_context_menu(self):
        selected_item = self.workspace.currentItem()
        if selected_item:
            file_name, ok = QInputDialog.getText(self, "Nuevo Archivo", "Nombre del archivo:", text="nuevo_archivo.txt")
            if ok and file_name:
                file_path = os.path.join(self.get_item_path(selected_item), file_name)
                self.create_file(file_path)
                self.add_structure_to_workspace(file_name)
    ...
    def get_item_path(self, item):
        # Obtener la ruta completa del ítem
        path = ""
        while item:
    ...
    def dropEvent(self, event):
        ...
        if mime_data.hasFormat('application/x-item'):
            item_data = mime_data.data('application/x-item')
            item_path = str(item_data, encoding='utf-8')
            if os.path.isfile(item_path):
                # Es un archivo
                self.create_file(item_path)
            else:
                # Es una carpeta
                self.create_folders([item_path], self.workspace.invisibleRootItem())
            event.accept()
        else:
            event.ignore()
...
if __name__ == '__main__':
    ...
    main_win = DragDropProjectOrganizer()
    main_win.show()
    sys.exit(app.exec_())
