...
class DragDropProjectOrganizer(QMainWindow):
    ...
    def create_folder(self):
        # Crear nueva carpeta en el área de trabajo
        item = QTreeWidgetItem(self.workspace, ["Nueva Carpeta"])
        item.setFlags(item.flags() | Qt.ItemIsEditable)
        self.workspace.addTopLevelItem(item)

    def rename_folder(self):
        selected_item = self.workspace.currentItem()
        if selected_item:
            old_name = selected_item.text(0)
            new_name, ok = QInputDialog.getText(self, "Renombrar Carpeta", "Nuevo nombre:", text=old_name)
            if ok and new_name:
    ...
    def delete_item_from_disk(self, item):
        # Borrar el archivo o carpeta del disco
